typedef CloudFunctionsResponseSuccessCallback<T> =
    T Function(CloudFunctionsResponseSuccess success);
typedef CloudFunctionsResponseFailureCallback<T> =
    T Function(CloudFunctionsResponseFailure failure);
...
sealed class CloudFunctionsResponse {
  ...
  T when<T>({
    ...
    final t = this;
    return switch (t) {
      CloudFunctionsResponseSuccess() => success(t),
      CloudFunctionsResponseFailure() => error(t),
    };
  }
...
class CloudFunctionsResponseFailure extends CloudFunctionsResponse
  ...
  CloudFunctionsResponseFailure.internalError({required super.functionName})
...
extension CloudFunctionsResponseExtension on Future<CloudFunctionsResponse> {
  Future<bool> toSuccessOrThrow() {
    return then(
      (value) =>
          value.when(success: (_) => true, error: (error) => throw error),
    );
  }
}
