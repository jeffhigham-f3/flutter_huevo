...
class FirestoreCollectionProvider<T> extends ItemProvider<T>
  ...
  Stream<List<T>> getAllStream() {
    return collection.snapshots().map(
      (snapshot) => snapshot.docs.map(fromDoc).toList(),
    );
  }
  ...
  @protected
  List<T> toItemList(QuerySnapshot<Map<String, dynamic>> snapshot) => snapshot
      .docs
      .map((e) {
        final item = fromDoc(e);
        if (item == null) {
          loggy.info('Item is null: ${e.data()}');
        }
